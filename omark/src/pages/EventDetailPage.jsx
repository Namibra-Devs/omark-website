...
import { Calendar, MapPin, Clock, ArrowLeft, Share2, Download, CheckCircle } from 'lucide-react';
  ...
  return (
    <section className="pt-32 pb-20 bg-gray-50 min-h-screen">
      <div className="max-w-4xl mx-auto px-5 sm:px-8">
        {/* Back Button */}
        <Link to="/events" className="inline-flex items-center gap-2 text-amber-600 hover:text-amber-700 mb-6 transition-colors">
          <ArrowLeft size={18} />
          Back to Events
        </Link>
        ...
        <div className="bg-white rounded-2xl overflow-hidden shadow-lg mb-8">
          <div className="relative h-80">
            <img
              ...
              onError={(e) => {
                e.target.src = 'https://placehold.co/1200x600/f59e0b/white?text=Event';
              }}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
            <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
              <div className="flex gap-2 mb-3">
                <span className="bg-amber-600 px-3 py-1 rounded-lg text-sm font-semibold">{event.category}</span>
                <span className="bg-green-600 px-3 py-1 rounded-lg text-sm font-semibold">{event.status}</span>
              </div>
          ...
          <div className="p-6 md:p-8">
            <div className="flex flex-wrap gap-4 justify-between items-start mb-6">
              <h2 className="text-2xl font-bold text-[#14141D]">About This Event</h2>
            ...
            {/* Agenda */}
            <div className="mb-8">
              <h3 className="text-xl font-bold text-[#14141D] mb-4">Event Agenda</h3>
              <div className="space-y-3">
                {event.agenda.map((item, idx) => (
                  <div key={idx} className="flex gap-4 p-3 bg-gray-50 rounded-lg">
                    <div className="w-24 font-semibold text-amber-600">{item.time}</div>
            ...
            <div className="bg-gradient-to-r from-amber-50 to-amber-100 rounded-xl p-6 text-center">
              <h3 className="text-xl font-bold text-[#14141D] mb-2">Ready to Join?</h3>
              <p className="text-gray-600 mb-4">Secure your spot at this exclusive event</p>
